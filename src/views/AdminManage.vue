<template>
  <div>
    <h1>当前课程代码</h1>
    <el-table :data="codeData" style="width:540px " id="table">
      ...
      <el-table-column prop="operation" label="操作" width="180">
        <template  #default="scope">
          ...
          <el-button type="danger" icon="Delete" circle @click="deleteCode(scope.$index)">
    ...
    <el-form :model="param" :rules="rules" ref="addcode" label-width="80px">
      ...
      <el-form-item label="课程代码" prop="code">
        <el-input type="text" placeholder="输入要添加的课程代码" size="small"
                  prefix-icon="el-icon-notebook-1"
                  v-model="param.code" style="width: 200px;"></el-input>
      </el-form-item>
      <el-form-item label="课程名称" prop="name">
        <el-input type="text" placeholder="输入要添加的课程名称" size="small"
                  prefix-icon="el-icon-lollipop"
                  v-model="param.name" style="width: 200px;"></el-input>
      </el-form-item>
      &nbsp;&nbsp;&nbsp;<el-button type="danger" @click="Submit">添加课程代码</el-button>
    </el-form>

    <el-dialog :visible="dialogFormVisible" title="修改课程代码"
      ...
      <el-form :model="form" :rules="rules2" ref="modifycode">
        <el-form-item label="课程代码" label-width="140px" prop="code">
          <el-input v-model="form.code"  prefix-icon="el-icon-notebook-1" autocomplete="off" />
        </el-form-item>
        <el-form-item label="课程名称" label-width="140px" prop="name">
          <el-input v-model="form.name"  prefix-icon="el-icon-lollipop" autocomplete="off" />
        </el-form-item>
...
export default {
  ...
  data(){
    return {
      ...
      rules: {
        code: [
          {required: true, message: "请输入要添加的课程代码", trigger: "blur"},
        ],
        name: [
          {required: true, message: "请输入要添加的课程名称", trigger: "blur"}
        ],
      },
      //修改课程代码规则
      rules2: {
  ...
  methods:{
    ...
    GetCode(){
      ...
      this.axios.post("showcode",params
      ).then((response) => {
        load.close()
    ...
    modifyCode(){
      ...
      if( this.codeData[index]['code'] ==  this.form.code && this.codeData[index]['name'] ==  this.form.name ){
        //没有什么变化
        this.$message({
          type: 'error',
    ...
    Submit(){
      ...
      this.$confirm('您要添加的课程为：'+this.param.code+"、"+this.param.name+"，是否确认无误", '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
